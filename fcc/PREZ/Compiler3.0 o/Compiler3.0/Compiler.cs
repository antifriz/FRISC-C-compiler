...
namespace Compiler1._0
{
    class Compiler
    {
        private bool IspraviFRISC3 = true;

        #region Variables

            public string InputFile = null;

            public string starter = "\t\tMOVE 1000,SP\n\t\tJR _start\n;\t\tDW IRQ_start\n;\t\tJR FIQ_start\n_start\n\t\tSUB SP,4,SP\n\t\tCALL main\n\t\tADD SP,4,SP\n\t\tHALT\n";
            ...
            private List<Variable> GlobalVariablesDeclared = new List<Variable>();
        ...
        public void Compile(string sourcePath,string destinationPath,string globalsPath,List<string> head)
        {
            ...
            AddGlobalVarsAndConsts(GlobalVariablesDeclared, parserOutputRaw);
        ...
        private void GlobalParse()
        {
            ...
            PreParsing(GlobalVariablesDeclared, f.Name);

            List<Command> Commands = new List<Command>();

            int i=GlobalVariablesDeclared.Count;

            using (StreamReader FunctionStream = FindFunction(f))
            {
                while (ParseDeclaration(FunctionStream, GlobalVariablesDeclared)) ;

                Started = false;

                while (ParseCommands(FunctionStream, GlobalVariablesDeclared, Commands)) ;
            }

            if (i < GlobalVariablesDeclared.Count)
                GlobalVariablesDeclared.RemoveAt(i);
            Variable v;
            i = 0;
            while (i < GlobalVariablesDeclared.Count)
            {
                for (; i < GlobalVariablesDeclared.Count; i++)
                {
                    if (GlobalVariablesDeclared[i].Value.IsNumber())
                        continue;
                    if ((v = FindVariable(GlobalVariablesDeclared, GlobalVariablesDeclared[i].Value)) == null)
                        continue;
                    v.Name = GlobalVariablesDeclared[i].Name;
                    GlobalVariablesDeclared.RemoveAt(i);
                    i--;
                    break;
                }
           ...
           // GlobalVariablesDeclared.RemoveAt(0);
            ...
            for (int j = 0; j < GlobalVariablesDeclared.Count; j++)
                GlobalVariablesDeclared[j].IsGlobal = true;
        }
        ...
        #region Parse Functions
            ...
            #region Parsing Functions
                ...
                private List<Command> ParseFunction(Function f)
                {
                    ...
                    VariablesDeclared.AddRange(GlobalVariablesDeclared);
                    Variable p;
                   ...
                   //ReduceJumps(Commands);
                    ...
                    FinishParsing(VariablesDeclared, Commands, f.Name);
                    //PrintBlank(Commands);
                    Print(VariablesDeclared);
                                    ...
                                    switch (op1[0])
                                    {
                                        ...
                                        case ('&'):
                                            ...
                                            if (op1.Contains('['))
                                            {
                                                ...
                                                {
                                                    System.Windows.Forms.MessageBox.Show("Smatrati cu " + op1 + " kao da pise " + op1.Remove(op1.IndexOf('[')) + " jer je 2 sata i idem spavati");
                                                }
                                                else
                                                {
                                        ...
                                        default:
                                            ...
                                            if (!op1.Contains('['))
                                            {
                                                ...
                                                else if (!Started&&InputFile.Contains("global"))
                                                {
                                                    Dest.Value = op1;
                                                }
                                                else
                ...
                private void FinishParsing(List<Variable> list, List<Command> line,string name)
                {
        ...
        private string AddAdditionalFunctions(string input)
        {
            ...
            foreach (string EmbeddedFunction in EmbeddedFunctionsFound)
            {
                ...
                else
                {
                    ...
                    {
                        starter = starter.Replace(";\t\tDW", "\t\tDW");
                        input = input.Replace(EmbeddedFunction + "  \n\t\tRET", buffer);
                        output = output.Insert(0, GenerateIRQCode());
                    }
                    else if (EmbeddedFunction == "InitFIQ")
                    {
                        starter = starter.Replace(";\t\tJR", "\t\tJR");
                        input = input.Replace(EmbeddedFunction + "  \n\t\tRET", buffer);
                        output = output.Insert(0, GenerateFIQCode());                
                    }
                    else
                    {
                    }
